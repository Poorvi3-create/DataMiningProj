...
def safe_silhouette(X, labels, preferred=400, rng=None):
    if rng is None:
    ...
    if n <= preferred:
        try:
            ...
            return -1.0
    try:
        idx = rng.choice(n, size=preferred, replace=False)
...
def main():
    ...
    # PCA (use 2 components to speed up)
    n_comp = min(2, train_s.shape[1])
    ...
    for k in ks:
        ...
        sils.append(safe_silhouette(train_pca, labels, preferred=400, rng=rng))

    best_idx = int(np.argmax(sils))
    best_k = list(ks)[best_idx]
    if all(s <= -0.5 for s in sils):
        best_k = 5
    ...
    print("Silhouette:", dict(zip(list(ks), sils)))
